...
void loop() {
    ...
    if (digitalRead(FOOTSWITCH) == LOW) { // FOOTSWITCH IS PRESSED
     ...
     volumeControl(); // Check volume control push-buttons every 50 iterations
...
//   DDRB |= ((PWM_QTY << 1) | 0x02); // turn on outputs
  ...
  // Enable PWM outputs on Mega (OCR1A = pin 11, OCR1B = pin 12)
...
counter++; //to save resources, the push-buttons are checked every 100mes.
if(counter==100)
{
counter=0;
if (!digitalRead(PUSHBUTTON_1)) {
  if (pot2_value<1024) pot2_value++; //increase vol
    }
if (!digitalRead(PUSHBUTTON_2)) {
  if (pot2_value>0) pot2_value--; //decrease vol
    }
}
}
